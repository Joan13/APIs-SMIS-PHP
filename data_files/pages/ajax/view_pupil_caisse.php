<?php
	...
	<div class="main_mm outCaisse">
		...
		if(session_in() == 6) {
			...
			<button type="button" value="<?=$pupil_id?>" class="validate_login" id="button_new_input">Enregistrer un nouveau reçu</button>
			<?php
		}
		...
		?>
		<button type="button" value="<?=$pupil_id?>" class="validate_login" id="voir_tous_les_recus">Reçus scolaires de l'élève</button>
					...
					<tr>
						<td  style="border: 1px solid rgba(0, 0, 0, 0.1); border-radius: 10px;">
							<div style="display: inline-block; text-align: left;">
								...
								<strong>Montant payé en dollars : </strong><br/><input type="number" id="input_montant" class="innput" placeholder="Ex : 120"><br/>
								...
								<select class="innput_ss" id="selectionner_le_libelle">
									<option value="">Sélectionner un libéllé</option>
									<?php
									$select_all011 = "SELECT * FROM libelles WHERE gender_libelle=1";
								...
								$years11->execute(array($yearr));
								$yearss = $years11->fetchObject();
								?>
											
								<button id="school_year_input" value="<?= $yearss->year_id ?>" style="background-color: transparent; border: none; font-weight: bold; display: inline-block;"><?= $yearss->year_name ?></button>
								<br/><button type="button" value="<?=$pupil_id?>" id="validate_paiement" class="validate_login">Valider le paiement</button><br/><br/>
							</div>
						</td>
						<td  style="border: 1px solid rgba(0, 0, 0, 0.1); border-radius: 10px;">
							<div style="display: inline-block; text-align: left;">
								<br/><strong>INITIALISER LE MONTANT TOTAL DE PAIEMENT PAR AN (CHOISIR L'ANNEE)</strong></h3><br/><br/>
								<strong>Montant total et année scolaire : </strong><br/><input type="number" id="total_montant_reset" class="ssss innput" placeholder="Ex : 540">
								<select class="ssss selects select_pupil_cycle" id="school_year_reset">
								...
								<br/><button type="button" value="<?=$pupil_id?>" id="validate_reset" class="validate_login">Valider l'initialisation</button><br/><br/>
							</div>
						</td>
					</tr>
				</table>
	...
	<div class="inCaisse">
		<a href="./?_=home.livre_caisse" class="inCaisse">REACTUALISER LE LIVRE DE CAISSE</a>
	</div>
		...
		$('#voir_tous_les_recus').on('click', function() {
			...
			if(ppupil != '' && yyearr != '')
			{
				$.post('pages/ajax/pupil_paiements.php', {ppupil:ppupil, yyearr:yyearr}, function(data) {
					...
					$('#frais_divers').fadeOut(-2200);
					$('#tous_les_recus').html(data);
					$('#tous_les_recus').fadeIn(200);
					$('#les_observations').fadeOut(-2200);
