...
$date_obs = htmlspecialchars(trim(strip_tags($_POST['date_day'])));

$countFraisDivers = "SELECT date_observation, COUNT(*) AS count_obss FROM observations WHERE date_observation=? AND deleted=0";
$requestCountFraisDivers = $database_connect->prepare($countFraisDivers);
$requestCountFraisDivers->execute(array($date_obs));
$responseCountFraisDivers = $requestCountFraisDivers->fetchObject();

if($responseCountFraisDivers->count_obss != 0) {

// $ssl0rrr = "SELECT * FROM pupils_info WHERE pupil_id=?";
// $ssl1rrr = $database_connect->prepare($ssl0rrr);
// $ssl1rrr->execute(array($pupil_id));
// $sslrrr = $ssl1rrr->fetchObject();

// $pupil_id = htmlspecialchars(trim(strip_tags($_POST['ppupil'])));
// $s_year = htmlspecialchars(trim(strip_tags($_POST['yyearr'])));

?>
<h2>Feuillet de discipline de la journée du <?= $date_obs ?></h2>
...
<tr style="border: 1px solid rgba(0, 0, 0, 0.2);">
    <th style="width: 150px; text-align: center; color: black;">Date</th>
    <th style=" max-width: 500px; color: black;">Détaillé de l'observation</th>
    <th style="width: 100px; text-align: center; color: black;">Mention</th>
    <th style="width: 150px; text-align: center; color: black;">Classe</th>
</tr>

<?php

$query = "SELECT * FROM observations WHERE deleted=? AND date_observation=?";
$request = $database_connect->prepare($query);
$request->execute(array(0, $date_obs));
while($response = $request->fetchObject()) {
    ...
    $ssl1->execute(array($response->pupil_id));
    ...
    $school_year = find_school_year($response->school_year);
    ...
    ?>
    <tr>
        <td style="border: 1px solid rgba(0, 0, 0, 0.2); text-align: center;"><?= strtoupper($ssl->first_name). " " .strtoupper($ssl->second_name) ?></td>
        ...
        <td style="border: 1px solid rgba(0, 0, 0, 0.2); text-align: center;"><?= $class_identity ?></td>
...
<?php
  ...
  ?>
  <br/><div>Il n'y a pas de feuillets d'observation disponibles pour cette date</div>
  <?php
