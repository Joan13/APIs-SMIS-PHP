$(document).ready(function() {
	...
	$('#open_new_teacher').on('click', function() {
		$('.divv_info').fadeToggle(-4000);
		$('.div_new_teacher').fadeToggle(-400);
	});
	...
	$('#close_div_migrations').on('click', function(){
		$('#migrations_div').fadeOut(400);
	});

	$('#validate_insert_worker').on('click', () => {
		let worker_names = $('#worker_names').val();
		let gender_worker = $('#gender_worker').val();
		let todo_worker = $('#todo_worker').val();
		let worker_year = $("#worker_year").val();

		if (worker_names == "" || todo_worker == "" || worker_year == "") {
	...
	$('#validate_insert_attribution').on('click', () => {
		let worker_id = $('#worker_id').val();
		let course_attributed = $('#course_attributed').val();

		if (worker_id == "" || course_attributed == "") {
			$('#information-div-global').fadeIn(400);
		} else {
			$.post('pages/ajax/attribution_course.php', 
			{ 
				worker_id:worker_id, 
				course_id:course_attributed, 
			}, 
			(data) => {
				$('#worker_id').val('');
				$('#course_attributed').val('');
	...
	$('#les_cours').on('click', function() {
	  ...
	  $('#submit_change_password').on('click', function(){
		...
		else
		{
		  ...
		  {
			$.post('pages/ajax/password_edit_worker.php', {worker_id:poste_number, old_password:old_password, new_password2:new_password2}, function(){
			  $('#success-information-div-global').fadeIn(400);
			});
		  }
		}
