...
<style>
	...
	<div id="print_fiche_points_draft" style="display: block !important; height: 0px;">
		...
		<div style="text-align: left;font-size: 12px;">
			...
			<strong>SEMESTRE :......(PERIODES/EXAMEN)</strong>
		</div>
		<strong style="text-align: center;font-size: 12px;">FICHE SYNTHESE DES POINTS : <?=$class_identity?></strong>
		</caption>
		<thead>
			<tr>
				<th style="text-align:left;padding-left: 5px;border: 1px solid black;font-size: 10px;">N0</th>
				<th style="text-align:left;padding-left: 5px;width: 500px;border: 1px solid black;font-size: 10px;">NOMS DE L'ELEVE</th>
				<?php 
				if(count_courses_exist($_GET['cycle'], $_GET['class_id'], $_GET['section_id'], $_GET['option_id'], $_GET['school_year']) != 0)
				{
					...
					while($query_fetch_cn = $query_fetch11_cn->fetchObject())
					{
						...
						?>
						<th style="text-align:center; border: 1px solid black;font-size: 10px;" colspan="3">
							<?= mb_strToUpper($query_fetch_cn->course_name." /".$query_fetch_cn->total_marks) ?>
						</th>
						<?php
					}
				}
			...
			<tr>
				...
				if(count_courses_exist($_GET['cycle'], $_GET['class_id'], $_GET['section_id'], $_GET['option_id'], $_GET['school_year']) != 0)
				{
					...
					while($query_fetch_cn = $query_fetch11_cn->fetchObject())
					{
						...
						?>
						<th style="text-align:center; border: 1px solid black;font-size: 10px;min-width: 20px;">
							PER
						</th>
						<th style="text-align:center; border: 1px solid black;font-size: 10px;min-width: 20px;">
							PER
			...
			if(count_pupils_exist($_GET['cycle'], $_GET['class_id'], $_GET['order_id'], $_GET['section_id'], $_GET['option_id'], $_GET['school_year']) != 0)
			{
				...
				while($query_fetch0 = $query_fetch110->fetchObject())
				{
					...
					?>
					<tr class="okokhover">
					<td style="text-align: left;font-weight: normal; padding-left: 5px;border: 1px solid black;font-size: 10px;">
					<?= $number ?>
					</td>
						<td style="text-align: left;font-weight: normal; padding-left: 5px;border: 1px solid black;height: 20px;">
